; Custom installer strings for Setlist.
;
; customLanguageFiles REPLACES Tauri's generated English.nsh rather than merging with it, so
; every LangString the template references has to be present here. These are Tauri's own
; strings verbatim except for deleteAppData at the bottom.
;
; If a Tauri upgrade adds a new LangString, the build will fail on the missing symbol -- diff
; this against target/release/nsis/x64/English.nsh and copy the new line across.
...
LangString deleteAppData ${LANG_ENGLISH} "Delete settings, unpushed edits and the saved Spotify login"
